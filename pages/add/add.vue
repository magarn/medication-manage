	...
	<view class="content">
		<view class="text-area">
			...
			<view class="textarea_from">
				<u-calendar v-model="alarm_show" :mode="mode" @change="change"></u-calendar>
				<u--form :model="loginForm" ref="uLoginForm">
					<view>
						<text class="title">今日药品备忘</text>
						<br />
						<p style="font-weight: bold;margin-top: 8px;">{{dateTimeStr(date,'y-m-d h:i:s')}}</p>
					</view>
					<u-form-item prop="title">
						<u-input v-model="loginForm.title" placeholder="药品名称" style="background-color: white" />
					</u-form-item>
					<u-form-item prop="dose">
						<u-input v-model="loginForm.dose" placeholder="药品剂量" style="background-color: white" />
					</u-form-item>
					<u-form-item prop="alarm">
						<u-datetime-picker :minDate="newDay" ref="datetimePicker" @confirm="confirm" :show="alarm_show" v-model="loginForm.alarm" :formatter="formatter" mode="datetime">
						</u-datetime-picker>
					...
					<u-form-item prop="content">
						<u--textarea v-model="loginForm.content" @focus="passwordF_B" @blur="passwordF_B" height=160
							count :maxlength="maxlength" placeholder="请输入提醒内容" border="bottom"></u--textarea>
					</u-form-item>
					<view class="btn_Box">
